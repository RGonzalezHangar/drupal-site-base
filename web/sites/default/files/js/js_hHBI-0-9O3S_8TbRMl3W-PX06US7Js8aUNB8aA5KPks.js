...
jQuery(document).ready(function() {
    ...
    JQuery('.optChinese').hide();
    
    JQuery('.country-CN').hide();
    JQuery('.country-SG').hide();
    JQuery('#selectLanguage').attr('disabled', 'disabled');
    JQuery('#selectLanguage').change(function(event) {
         console.log('changed language', JQuery(this).val());
         
         if (JQuery('#selectCountry').val() == 'CA' || JQuery('#selectCountry').val() == 'SG') {
             JQuery('li.all a').each(function(i,e) {
...
//                     console.log(href);
        ...
        JQuery('.all, .language-list h1').hide();
        JQuery('.content-list li').hide();
        toggleLanguage(this.value);
    });

    //country
    JQuery('#selectCountry').change(function(event) {
        // console.log('changed country', JQuery(this).val());
        var countryCode = JQuery(this).val();
        if ('all' == countryCode) {
            JQuery('.country-specific').hide();
            JQuery('.country-not-specific').show();
            JQuery('.all').css('visibility','visible'); 
        } else {
            JQuery('.country-' + countryCode).show();
            JQuery('.country-not-' + countryCode).hide();
        }

        JQuery('.all, .language-list h1').hide();
        JQuery('.content-list li').hide();
        if (JQuery('.prompt.' + this.value).length > 0) {
            JQuery('.prompt.' + this.value).fadeIn('slow');
            JQuery('#selectLanguage').attr('disabled', 'disabled');
        }
        else {
            JQuery('.language-list h1').show();
            JQuery('.pt').hide();
            JQuery('#selectLanguage').removeAttr('disabled');
            toggleLanguage(JQuery('#selectLanguage').val());
        }


        if(countryCode === 'CN' || countryCode === 'SG'){
            JQuery('.optChinese').show();
        } else {
            JQuery('.optChinese').hide();
        }
...
//                     console.log(href);
        ...
        if(JQuery('#selectLanguage').val() === 'chinese' && (countryCode !== 'CN' && countryCode !== 'SG')){
            JQuery('#selectLanguage').val('all');
            JQuery('.all, .language-list h1').hide();
            JQuery('.content-list li').hide();
            toggleLanguage('all');
    ...
    JQuery('.it').change(function(event) {
        JQuery('.all, .language-list h1').hide();
        JQuery('.prompt').fadeIn('slow');
    });

    JQuery('.prompt .accept').click(function() {
        toggleLanguage(JQuery('#selectLanguage').val());
        JQuery('#selectLanguage').removeAttr('disabled');
    });
    JQuery('.prompt .reject').click(function() {
        JQuery('.prompt-one').hide();
        JQuery('.reject-message', JQuery(this).parents('.prompt')).fadeIn('slow');
    });
...
function toggleLanguage(language) {
    ...
    JQuery('.prompt').hide();
    JQuery('.prompt-one').show();
    JQuery('.reject-message').hide();

    JQuery('.language-list h1').each(function() {
        if (JQuery(this).next('ul').find('li:visible').length === 0) {
            JQuery(this).hide();
        } else {
            JQuery(this).show();
        }
